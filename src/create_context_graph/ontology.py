...
class EntityTypeDef(BaseModel):
    """A domain entity type (maps to a LadybugDB node table)."""
...
def generate_cypher_schema(ontology: DomainOntology) -> str:
    """Generate LadybugDB DDL schema (CREATE NODE TABLE / CREATE REL TABLE) from the ontology."""
    # Map ontology property types to LadybugDB types
    type_map = {
        "string": "STRING",
        "integer": "INT64",
        "float": "DOUBLE",
        ...
        "date": "DATE",
        "datetime": "TIMESTAMP",
        "point": "STRING",  # LadybugDB doesn't have a native point type; store as WKT string
    }

    lines = [
        "// Auto-generated LadybugDB schema for domain: " + ontology.domain.name,
        "// Generated by create-context-graph",
        "",
    ]

    # Node tables for domain entity types
    for et in ontology.entity_types:
        cols = []
        has_name = False
        for prop in et.properties:
            lb_type = type_map.get(prop.type, "STRING")
            cols.append(f"    {prop.name} {lb_type}")
            if prop.name == "name":
                has_name = True
        # Ensure every node table has a name column (used as primary key)
        if not has_name:
            cols.insert(0, "    name STRING")
        # Add a description column if not present
        has_desc = any(p.name == "description" for p in et.properties)
        if not has_desc:
            cols.append("    description STRING")
        col_str = ",\n".join(cols)
        lines.append(f"CREATE NODE TABLE IF NOT EXISTS {et.label} (")
        lines.append(col_str + ",")
        lines.append("    PRIMARY KEY (name)")
        lines.append(");")
        lines.append("")

    # Infrastructure node tables: Document, DecisionTrace, TraceStep
    lines.append("// Infrastructure: Document and Decision Trace tables")
    lines.append("CREATE NODE TABLE IF NOT EXISTS Document (")
    lines.append("    title STRING,")
    lines.append("    content STRING,")
    lines.append("    template_id STRING,")
    lines.append("    template_name STRING,")
    lines.append("    PRIMARY KEY (title)")
    lines.append(");")
    lines.append("")
    lines.append("CREATE NODE TABLE IF NOT EXISTS DecisionTrace (")
    lines.append("    id STRING,")
    lines.append("    task STRING,")
    lines.append("    outcome STRING,")
    lines.append("    PRIMARY KEY (id)")
    lines.append(");")
    lines.append("")
    lines.append("CREATE NODE TABLE IF NOT EXISTS TraceStep (")
    lines.append("    trace_step_id STRING,")
    lines.append("    trace_id STRING,")
    lines.append("    step_number INT64,")
    lines.append("    thought STRING,")
    lines.append("    action STRING,")
    lines.append("    observation STRING,")
    lines.append("    PRIMARY KEY (trace_step_id)")
    lines.append(");")
    lines.append("")

    # Conversation memory tables (built-in conversation memory)
    lines.append("// Conversation memory tables")
    lines.append("CREATE NODE TABLE IF NOT EXISTS ConversationMessage (")
    lines.append("    msg_id STRING,")
    lines.append("    session_id STRING,")
    lines.append("    role STRING,")
    lines.append("    content STRING,")
    lines.append("    created_at TIMESTAMP,")
    lines.append("    PRIMARY KEY (msg_id)")
    lines.append(");")
    lines.append("")
    ...
    # Infrastructure relationship tables
    lines.append("CREATE REL TABLE IF NOT EXISTS MENTIONS (FROM Document TO Person);")
    lines.append("CREATE REL TABLE IF NOT EXISTS HAS_STEP (FROM DecisionTrace TO TraceStep);")
    lines.append("")
